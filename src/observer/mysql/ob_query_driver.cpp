...
int ObQueryDriver::response_query_header(ObResultSet &result,
  ...
  if (is_prexecute_) {
    ...
    if (OB_FAIL(static_cast<ObMPStmtPrexecute &>(sender_).response_query_header(
            session_, result, need_flush_buffer))) {
    ...
    } else if (OB_FAIL(response_query_header(
                   *result.get_field_columns(), has_more_result,
                   need_set_ps_out_flag, false, &result))) {
...
int ObQueryDriver::response_query_header(const ColumnsFieldIArray &fields,
  ...
  if (OB_SUCC(ret)) {
    for (int64_t i = 0; OB_SUCC(ret) && i < fields.count(); ++i) {
      ...
      if (NULL != result && result->get_is_com_filed_list() &&
        ...
        if (session_.is_support_new_result_meta_data() ||
            tmp_ret != OB_SUCCESS) {
  ...
  if (OB_SUCC(ret)) {
    ...
    // NULL == result indicates it is an old protocol ps cursor execute
    // response, or fetch protocol response, cursor_exit = true
    flags.status_flags_.OB_SERVER_STATUS_CURSOR_EXISTS = NULL == result ? 1 : 0;
    if (!session_.is_obproxy_mode()) {
      // in java client or others, use slow query bit to indicate partition hit
      // or not
      flags.status_flags_.OB_SERVER_QUERY_WAS_SLOW =
          !session_.partition_hit().get_bool();
    }
    eofp.set_server_status(flags);

    if (ps_cursor_execute && sender_.need_send_extra_ok_packet()) {
      // Old protocol ps cursor execute response, only returns field
      // information, so for proxy, an additional OK packet needs to be returned
      // However, since the 2.0 protocol needs to understand the EOF packet
      // situation at the same time as sending an OK packet, this OK packet
      // cannot be extracted to the execute protocol layer for processing
      if (OB_FAIL(sender_.update_last_pkt_pos())) {
        LOG_WARN("failed to update last packet pos", K(ret));
      } else {
        // in multi-stmt, send extra ok packet in the last stmt(has no more
        // result)
        ObOKPParam ok_param;
...
int ObQueryDriver::response_query_result(ObResultSet &result,
                                         bool is_ps_protocol,
                                         bool has_more_result, bool &can_retry,
                                         int64_t fetch_limit) {
  int ret = OB_SUCCESS;
  ...
  bool is_packed = result.get_physical_plan()
                       ? result.get_physical_plan()->is_packed()
                       : false;
  MYSQL_PROTOCOL_TYPE protocol_type =
      is_ps_protocol ? MYSQL_PROTOCOL_TYPE::BINARY : MYSQL_PROTOCOL_TYPE::TEXT;
  ...
  _OB_LOG(INFO,
          "[QUERY_CACHE_WRITE] response_query_result called, "
          "is_ps_protocol=%d, is_prexecute=%d, sql_ctx=%p",
          is_ps_protocol, is_prexecute_, sql_ctx);
  ...
  if (query_cache.is_inited() && OB_NOT_NULL(sql_ctx) && !is_ps_protocol &&
    ...
    _OB_LOG(INFO,
            "[QUERY_CACHE_WRITE] checking cacheable, sql_len=%ld, sql=%.*s",
            sql.length(),
            sql.length() > 50 ? 50 : static_cast<int>(sql.length()), sql.ptr());
    if (sql::ObQueryResultCache::is_cacheable_sql(sql_std_string)) {
      cache_key = sql::ObQueryCacheKey(sql_std_string);
      // 检查缓存中是否已存在（避免重复写入）
      sql::ObCachedQueryResult *existing = nullptr;
      if (OB_SUCCESS != query_cache.get(cache_key, existing)) {
        // 缓存中不存在，准备写入
        should_cache = true;
        cache_to_write = query_cache.alloc_result();
        _OB_LOG(
            INFO,
            "[QUERY_CACHE_WRITE] will cache result, alloc_result=%p, sql=%.*s",
            cache_to_write,
            sql.length() > 100 ? 100 : static_cast<int>(sql.length()),
            sql.ptr());
      } else {
        _OB_LOG(INFO, "[QUERY_CACHE_WRITE] already cached, skip");
      }
    } else {
      _OB_LOG(INFO, "[QUERY_CACHE_WRITE] sql not cacheable");
    }
  } else {
    _OB_LOG(INFO,
  ...
  if (OB_FAIL(ret)) {
    ...
    if (!result.get_has_top_limit() &&
        OB_FAIL(session_.get_sql_select_limit(limit_count))) {
  ...
  // 如果 limit 太大，不缓存
  // if (should_cache && limit_count > MAX_CACHEABLE_ROWS) {
  //   should_cache = false;
  //   if (OB_NOT_NULL(cache_to_write)) {
  ...
  if (OB_SUCC(ret) && 0 == row_num) {
    // If there is no data at all, we still need to reply to the client with
    // field information, and no more retries will be attempted
    can_retry = false;
    if (OB_FAIL(response_query_header(result, has_more_result, false))) {
      LOG_WARN("fail to response query header", K(ret), K(row_num),
               K(can_retry));
  ...
  if (OB_FAIL(ret) && !can_retry) {
    FLOG_INFO("The query has already returned partial results to the client "
              "and cannot be retried",
              KR(ret));
  }

  // ========== 查询完成后，将结果写入缓存 ==========
  if (OB_SUCC(ret) && should_cache && OB_NOT_NULL(cache_to_write)) {
    int cache_ret = query_cache.put(cache_key, cache_to_write);
    if (OB_SUCCESS != cache_ret) {
      _OB_LOG(INFO, "[QUERY_CACHE] put to cache FAILED, ret=%d", cache_ret);
      query_cache.free_result(cache_to_write);
    } else {
      _OB_LOG(INFO, "[QUERY_CACHE] result cached SUCCESSFULLY, row_count=%ld, "
                    "total_entries=%ld");
    }
    cache_to_write = nullptr; // 已经交给缓存管理
  } else if (OB_NOT_NULL(cache_to_write)) {
    // 查询失败或不需要缓存，释放缓存对象
    query_cache.free_result(cache_to_write);
    cache_to_write = nullptr;
  }
...
int ObQueryDriver::convert_field_charset(ObIAllocator &allocator,
  ...
  if (0 == buf_len) {
    ...
    LOG_ERROR("alloc memory failed", K(ret));
                 ...
                 from_string.ptr(), from_string.length(), dest_collation, buf,
                 buf_len, result_len))) {
    LOG_WARN("charset convert failed", K(ret), K(from_collation),
             K(dest_collation));
  } else {
...
int ObQueryDriver::convert_string_value_charset(ObObj &value,
                                                ...
                                                ObCharsetType nchar) {
  int ret = OB_SUCCESS;
  ObCollationType to_collation_type =
  ...
  if (OB_FAIL(ret)) {
    ...
    const ObCharsetInfo *charset_info =
        ObCharset::get_charset(from_collation_type);
    if (OB_ISNULL(charset_info)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("charsetinfo is null", K(ret), K(from_collation_type),
               K(to_collation_type), K(value));
    } else if (CS_TYPE_INVALID == from_collation_type) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("invalid collation", K(ret), K(from_collation_type),
               K(to_collation_type), K(value));
    }
  } else if (OB_FAIL(result.get_exec_context().get_convert_charset_allocator(
                 allocator))) {
...
int ObQueryDriver::convert_text_value_charset(common::ObObj &value,
  ...
  ObArenaAllocator *allocator = NULL;
  if (OB_FAIL(
          result.get_exec_context().get_convert_charset_allocator(allocator))) {
    LOG_WARN("fail to get lob fake allocator", K(ret));
  } else if (OB_ISNULL(allocator)) {
    ret = OB_ERR_UNEXPECTED;
  ...
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(convert_text_value_charset(value, charset_type, *allocator,
                                                &my_session,
...
int ObQueryDriver::like_match(const char *str, int64_t length_str, int64_t i,
                              const char *pattern, int64_t length_pat,
                              int64_t j, bool &is_match) {
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(str) || OB_ISNULL(pattern) ||
    ...
    ++j;
    if (OB_FAIL(
            like_match(str, length_str, i, pattern, length_pat, j, is_match))) {
      LOG_WARN("failed to match", K(ret));
    } else if (!is_match && i < length_str) {
      ++i;
      --j;
      if (OB_FAIL(like_match(str, length_str, i, pattern, length_pat, j,
                             is_match))) {
        LOG_WARN("failed to match", K(ret));
      }
    }
    ...
    if (OB_FAIL(
      ...
      LOG_WARN("failed to match", K(ret));
             ...
             tolower(str[i]) == tolower(pattern[j])) {
    ++i;
    ++j;
    if (OB_FAIL(
            like_match(str, length_str, i, pattern, length_pat, j, is_match))) {
...
int ObQueryDriver::convert_lob_locator_to_longtext(ObObj &value,
  ...
  // If the client uses the old lob (without locator header, only data), then
  // return the old lob
...
int ObQueryDriver::process_lob_locator_results(ObObj &value,
  ...
  if (OB_FAIL(
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("lob fake allocator is null.", K(ret), K(value));
  } else if (OB_FAIL(process_lob_locator_results(
                 value, session_.is_client_use_lob_locator(),
                 session_.is_client_support_lob_locatorv2(), allocator,
                 &result.get_session(), &result.get_exec_context()))) {
    LOG_WARN("convert lob to longtext fail.", K(ret), K(value));
  }
  return ret;
}

int ObQueryDriver::process_lob_locator_results(
    ObObj &value, bool is_use_lob_locator, bool is_support_outrow_locator_v2,
    ObIAllocator *allocator, const sql::ObSQLSessionInfo *session_info,
    sql::ObExecContext *exec_ctx) {
  int ret = OB_SUCCESS;
  // 1. if client is_use_lob_locator, return lob locator
  // 2. if client is_use_lob_locator, but not support outrow lob, return lob
  // locator with inrow data
  //    refer to sz/aibo1m
  // 3. if client does not support use_lob_locator ,,return full lob data
  // without locator header
  bool is_lob_type = value.is_lob() || value.is_json() || value.is_geometry() ||
  ...
  if (!is_lob_type) {
    ...
    if (loc.is_null()) { // maybe v1 empty lob
      ...
      if (OB_FAIL(ObTextStringHelper::build_text_iter(
              instr_iter, exec_ctx, session_info, allocator, &tmp_alloc))) {
        ...
        value.set_lob_value(dst_type, data.ptr(),
...
int ObQueryDriver::convert_string_charset(const ObString &in_str,
                                          const ObCollationType in_cs_type,
                                          const ObCollationType out_cs_type,
                                          char *buf, int32_t buf_len,
                                          uint32_t &result_len) {
  int ret = OB_SUCCESS;
  ret = ObCharset::charset_convert(in_cs_type, in_str.ptr(), in_str.length(),
                                   out_cs_type, buf, buf_len, result_len);
  if (OB_SUCCESS != ret) {
    int32_t str_offset = 0;
    int64_t buf_offset = 0;
    ObString question_mark = ObCharsetUtils::get_const_str(out_cs_type, '?');
    while (str_offset < in_str.length() &&
           buf_offset + question_mark.length() <= buf_len) {
      ...
      ret = ObCharset::charset_convert(in_cs_type, in_str.ptr() + str_offset,
                                       offset, out_cs_type, buf + buf_offset,
                                       buf_len - buf_offset, result_len);
      str_offset += offset;
      if (OB_SUCCESS == ret) {
    ...
    if (str_offset < in_str.length()) {
      ret = OB_SIZE_OVERFLOW;
      LOG_WARN("sizeoverflow", K(ret), K(in_str),
               KPHEX(in_str.ptr(), in_str.length()));
    } else {
...
int ObQueryDriver::convert_text_value_charset(
    ObObj &value, ObCharsetType charset_type, ObIAllocator &allocator,
    const sql::ObSQLSessionInfo *session, sql::ObExecContext *exec_ctx) {
  int ret = OB_SUCCESS;
  ...
  if (value.is_null() || value.is_nop_value()) {
    ...
    if (!value.has_lob_header() || !value.is_lob_storage()) {
      ...
      LOG_WARN("Lob: get  empty or null lob obj with header", K(ret), K(value));
             ...
             CHARSET_BINARY != charset_type) {
    ObCollationType to_collation_type =
        ObCharset::get_default_collation(charset_type);
    ObCollationType from_collation_type = value.get_collation_type();
    const ObCharsetInfo *from_charset_info =
        ObCharset::get_charset(from_collation_type);
    const ObCharsetInfo *to_charset_info =
        ObCharset::get_charset(to_collation_type);
    const ObObjType type = value.get_type();

    if (OB_ISNULL(from_charset_info) || OB_ISNULL(to_charset_info)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("Lob: charsetinfo is null", K(ret), K(from_collation_type),
               K(to_collation_type));
    } else if (CS_TYPE_INVALID == from_collation_type ||
               CS_TYPE_INVALID == to_collation_type) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("Lob: invalid collation", K(from_collation_type),
               K(to_collation_type), K(ret));
    } else if (CS_TYPE_BINARY != from_collation_type &&
               CS_TYPE_BINARY != to_collation_type &&
               strcmp(from_charset_info->csname, to_charset_info->csname) !=
                   0) {
      bool is_actual_return_lob_locator =
          session->is_client_use_lob_locator() && !value.is_json();
      if (value.is_lob_storage() && value.has_lob_header() &&
          OB_NOT_NULL(session) && is_actual_return_lob_locator &&
          lib::is_oracle_mode()) {
        ObLobLocatorV2 lob;
        ObString inrow_data;
        if (OB_FAIL(process_lob_locator_results(
                value, session->is_client_use_lob_locator(),
                session->is_client_support_lob_locatorv2(), &allocator, session,
                exec_ctx))) {
          ...
          int64_t offset_len = reinterpret_cast<uint64_t>(inrow_data.ptr()) -
                               reinterpret_cast<uint64_t>(lob.ptr_);
          int64_t res_len =
              offset_len + lob_data_byte_len * ObCharset::CharConvertFactorNum;
            ...
            if (OB_FAIL(convert_string_charset(
              ...
              if (OB_SUCC(ret)) {
                ...
                if (OB_FAIL(lob.get_disk_locator(lob_common))) {
                  ...
                  ObLobData *lob_data =
                      reinterpret_cast<ObLobData *>(lob_common->buffer_);
                  lob_data->byte_size_ = result_len;
                }
              }
              if (OB_SUCC(ret)) {
                LOG_DEBUG("Lob: new temp convert_text_value_charset in "
                          "convert_text_value",
                          K(ret), K(raw_str), K(type), K(to_collation_type),
                          K(from_collation_type), K(from_charset_info->csname),
                          K(to_charset_info->csname));
        ...
        // get full data, buffer size is full byte length *
        // ObCharset::CharConvertFactorNum
        ...
        if (!value.has_lob_header()) {
          ...
          // because the final result will be allocated by allocator when
          // convert charset
          if (OB_FAIL(ObTextStringHelper::build_text_iter(
                  str_iter, exec_ctx, session, &tmp_alloc /*res_allocator*/,
        ...
        if (OB_SUCC(ret)) {
          ...
          ObTextStringResult new_tmp_lob(type, value.has_lob_header(),
                                         &allocator);
          char *buf = NULL;
          int64_t buf_len = 0;
          uint32_t result_len = 0;
          int64_t converted_len =
              lob_data_byte_len * ObCharset::CharConvertFactorNum;
          if (OB_FAIL(new_tmp_lob.init(converted_len))) {
            ...
            LOG_WARN("Lob: get empty buffer failed", K(ret), K(converted_len));
                         ...
                         data_str, from_collation_type, to_collation_type, buf,
                         buf_len, result_len))) {
            LOG_WARN("Lob: convert string charset failed", K(ret));
...
/*@brief: is_com_filed_list_match_wildcard_str is used to match the parameters
 * in the COM_FIELD_LIST sent by the client that contain wildcard characters
 * Scenario, e.g.: COM_FIELD_LIST(t1, c*) , t1 has columns c1, c2, pk ==> only
 * return c1, c2, do not return pk, because it does not match c*; The rule is
 * similar to the like scenario; for details, refer to the link:
 *
 */
int ObQueryDriver::is_com_filed_list_match_wildcard_str(
    ObResultSet &result, const ObCollationType &from_collation,
    const ObString &from_string, bool &is_not_match) {
  int ret = OB_SUCCESS;
  is_not_match = false;
  if (!result.get_is_com_filed_list() || result.get_wildcard_string().empty()) {
    /*do nothing*/
  } else {
    /*Need to consider converting to the same character set for comparison when
     * comparing between different character sets*/
    ObIAllocator &allocator = result.get_mem_pool();
    ObString wildcard_str;
    if (result.get_session().get_nls_collation() != from_collation) {
      if (OB_FAIL(convert_field_charset(
              allocator, result.get_session().get_nls_collation(),
              from_collation, result.get_wildcard_string(), wildcard_str))) {
    ...
    if (OB_SUCC(ret)) {
      bool is_match = false;
      if (OB_FAIL(like_match(from_string.ptr(), from_string.length(), 0,
                             wildcard_str.ptr(), wildcard_str.length(), 0,
                             is_match))) {
