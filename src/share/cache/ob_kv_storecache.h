...
template <class Key, class Value> class ObIKVCache {
public:
  virtual int put(const Key &key, const Value &value,
                  bool overwrite = true) = 0;
  virtual int put_and_fetch(const Key &key, const Value &value,
                            const Value *&pvalue, ObKVCacheHandle &handle,
                            bool overwrite = true) = 0;
  virtual int get(const Key &key, const Value *&pvalue,
                  ObKVCacheHandle &handle) = 0;
  virtual int erase(const Key &key) = 0;
  virtual int alloc(const uint64_t tenant_id, const int64_t key_size,
                    const int64_t value_size, ObKVCachePair *&kvpair,
                    ObKVCacheHandle &handle,
                    ObKVCacheInstHandle &inst_handle) = 0;
  virtual int put_kvpair(ObKVCacheInstHandle &inst_handle,
...
public:
  ...
  int init(const char *cache_name, const int64_t priority = 1,
           const int64_t mem_limit_pct = 100);
  void destroy();
  ...
  virtual int get(const Key &key, const Value *&pvalue,
                  ObKVCacheHandle &handle);
  virtual int non_const_get(const Key &key, Value *&pvalue,
                            ObKVCacheHandle &handle);
...
public:
  ...
  int get_cache_inst_info(const uint64_t tenant_id,
                          ObIArray<ObKVCacheInstHandle> &inst_handles);
  int get_memblock_info(const uint64_t tenant_id,
                        ObIArray<ObKVCacheStoreMemblockInfo> &memblock_infos);
  ...
  int set_hold_size(const uint64_t tenant_id, const char *cache_name,
                    const int64_t hold_size);
  int get_hold_size(const uint64_t tenant_id, const char *cache_name,
                    int64_t &hold_size);
  ...
  int get_batch_data_block_cache_key(
      ObIArray<blocksstable::ObMicroBlockCacheKey> &keys) {
    return map_.get_batch_data_block_cache_key(
        DEFAULT_ONCE_BATCH_GET_BUCKET_NUM, keys);
  }
...
private:
  ...
  int register_cache(const char *cache_name, const int64_t priority,
                     const int64_t mem_limit_pct, int64_t &cache_id);
  ...
  int put(const int64_t cache_id, const ObIKVCacheKey &key,
          const ObIKVCacheValue &value, const ObIKVCacheValue *&pvalue,
          HazptrHolder &hazptr_holder, bool overwrite = true);
  int put(ObIKVCacheStore &store, const int64_t cache_id,
          const ObIKVCacheKey &key, const ObIKVCacheValue &value,
          const ObIKVCacheValue *&pvalue, HazptrHolder &hazptr_holder,
          bool overwrite = true);
  int alloc(const int64_t cache_id, const uint64_t tenant_id,
            const int64_t key_size, const int64_t value_size,
            ObKVCachePair *&kvpair, HazptrHolder &hazptr_holder,
  ...
  //     ObKVCacheInstHandle &inst_handle);
  int alloc(ObIKVCacheStore &store, const int64_t cache_id,
            const uint64_t tenant_id, const int64_t key_size,
            const int64_t value_size, ObKVCachePair *&kvpair,
            HazptrHolder &hazptr_holder, ObKVCacheInstHandle &inst_handle);
  int get(const int64_t cache_id, const ObIKVCacheKey &key,
          const ObIKVCacheValue *&pvalue, HazptrHolder &hazptr_holder);
...
private:
  ...
  static const int64_t DEFAULT_MAX_CACHE_SIZE =
      1024L * 1024L * 1024L * 1024L;                    // 1T
  ...
  static const int64_t BASE_SERVER_MEMORY_FACTOR =
      1L << 31; // 2G is the start level
...
private:
  ...
  public:
    ...
    void runTimerTask() { ObKVGlobalCache::get_instance().replace_map(); }
  };
...
public:
  ...
  inline ObKVMemBlockHandle *get_mb_handle() const {
    return hazptr_holder_.get_mb_handle();
  }
  inline void set_hazptr_holder(HazptrHolder &hazptr_holder) {
    this->hazptr_holder_.move_from(hazptr_holder);
  }
  bool need_trace() const;
  storage::ObStorageCheckID get_check_id() const {
    return static_cast<storage::ObStorageCheckID>(
        get_mb_handle()->inst_->cache_id_);
  }
...
public:
  ...
  /**
   * get a kvpair from the kvcache, if return OB_SUCCESS, remember to call
   * revert(handle) to revert the handle.
   * @param key: out
   * @param value: out
   * @param handle: out
   ...
   */
  template <class Key, class Value>
  int get_next_kvpair(const Key *&key, const Value *&value,
                      ObKVCacheHandle &handle);
...
template <class Key, class Value>
int ObIKVCache<Key, Value>::put_kvpair(ObKVCacheInstHandle &inst_handle,
                                       ObKVCachePair *kvpair,
                                       ObKVCacheHandle &handle,
                                       bool overwrite) {
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(NULL == kvpair) || OB_UNLIKELY(NULL == kvpair->key_) ||
      OB_UNLIKELY(NULL == kvpair->value_) || OB_UNLIKELY(!handle.is_valid())) {
    ...
    if (OB_ISNULL(inst_handle.get_inst())) {
      ...
      COMMON_LOG(WARN, "The inst is NULL, ", K(ret));
                   ...
                   *inst_handle.get_inst(), *kvpair->key_, kvpair,
...
template <class Key, class Value> ObKVCache<Key, Value>::~ObKVCache() {
  destroy();
}
...
int ObKVCache<Key, Value>::init(const char *cache_name, const int64_t priority,
  ...
  if (OB_UNLIKELY(inited_)) {
    ...
    COMMON_LOG(WARN, "The ObKVCache has been inited, ", K(ret));
             ...
             OB_UNLIKELY(priority <= 0 || mem_limit_pct <= 0 ||
                         mem_limit_pct > 100)) {
    ret = OB_INVALID_ARGUMENT;
    COMMON_LOG(WARN, "Invalid argument, ", KP(cache_name), K(priority), K(ret));
                 ...
                 cache_name, priority, mem_limit_pct, cache_id_))) {
    COMMON_LOG(WARN, "Fail to register cache, ", K(ret));
  } else {
    COMMON_LOG(INFO, "Succ to register cache", K(cache_name), K(priority),
               K_(cache_id));
    inited_ = true;
  }
  return ret;
...
template <class Key, class Value> void ObKVCache<Key, Value>::destroy() {
...
int ObKVCache<Key, Value>::set_priority(const int64_t priority) {
  ...
  } else if (OB_FAIL(ObKVGlobalCache::get_instance().set_priority(cache_id_,
                                                                  priority))) {
...
int ObKVCache<Key, Value>::set_mem_limit_pct(const int64_t mem_limit_pct) {
  ...
  } else if (OB_FAIL(ObKVGlobalCache::get_instance().set_mem_limit_pct(
                 cache_id_, mem_limit_pct))) {
...
int64_t ObKVCache<Key, Value>::size(const uint64_t tenant_id) const {
  ...
  if (OB_LIKELY(inited_)) {
    ...
    ObKVCacheInstHandle inst_handle;
    if (OB_SUCC(ObKVGlobalCache::get_instance().insts_.get_cache_inst(
...
int ObKVCache<Key, Value>::get_iterator(ObKVCacheIterator &iter) {
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    COMMON_LOG(WARN, "The ObKVCache has not been inited, ", K(ret));
                 ...
                 iter.init(cache_id_, &ObKVGlobalCache::get_instance().map_))) {
...
template <class Key, class Value>
int ObKVCache<Key, Value>::put(const Key &key, const Value &value,
                               bool overwrite) {
  int ret = OB_SUCCESS;
  ObKVCacheHandle handle;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    COMMON_LOG(WARN, "The ObKVCache has not been inited, ", K(ret));
  } else if (OB_FAIL(ObKVGlobalCache::get_instance().put(
                 cache_id_, key, value, pvalue, handle.hazptr_holder_,
                 overwrite))) {
    if (OB_ENTRY_EXIST != ret) {
      COMMON_LOG(WARN, "Fail to put kv to ObKVGlobalCache, ", K_(cache_id),
                 K(ret));
    }
  }
  return ret;
...
template <class Key, class Value>
int ObKVCache<Key, Value>::put_and_fetch(const Key &key, const Value &value,
                                         const Value *&pvalue,
                                         ObKVCacheHandle &handle,
                                         bool overwrite) {
  int ret = OB_SUCCESS;
...
template <class Key, class Value>
int ObKVCache<Key, Value>::get(const Key &key, const Value *&pvalue,
                               ObKVCacheHandle &handle) {
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    handle.reset();
    if (OB_FAIL(ObKVGlobalCache::get_instance().get(cache_id_, key, value,
...
template <class Key, class Value>
int ObKVCache<Key, Value>::non_const_get(const Key &key, Value *&pvalue,
                                         ObKVCacheHandle &handle) {
  int ret = OB_SUCCESS;
  const ObIKVCacheValue *value = NULL;
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (OB_FAIL(ObKVGlobalCache::get_instance().get(cache_id_, key, value,
      ...
      pvalue = const_cast<Value *>(reinterpret_cast<const Value *>(value));
      storage::ObStorageLeakChecker::get_instance().handle_hold(&handle);
    }
  }
  return ret;
...
int ObKVCache<Key, Value>::erase(const Key &key) {
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    COMMON_LOG(WARN, "Fail to erase key from ObKVGlobalCache, ", K_(cache_id),
               K(ret));
  }
  return ret;
}
...
template <class Key, class Value>
int ObKVCache<Key, Value>::alloc(const uint64_t tenant_id,
                                 const int64_t key_size,
                                 const int64_t value_size,
                                 ObKVCachePair *&kvpair,
                                 ObKVCacheHandle &handle,
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    COMMON_LOG(WARN, "The ObKVCache has not been inited, ", K(ret));
                 ...
                 cache_id_, tenant_id, key_size, value_size, kvpair,
                 handle.hazptr_holder_, inst_handle))) {
 ...
 */
template <class Key, class Value>
int ObKVCacheIterator::get_next_kvpair(const Key *&key, const Value *&value,
                                       ObKVCacheHandle &handle) {
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    while (OB_SUCC(ret)) {
      if (pos_ >= map_->bucket_num_ && handle_list_.empty()) {
        ...
        if (OB_FAIL(handle.hazptr_holder_.protect(
                protect_success, node.mb_handle_, node.seq_num_))) {
        ...
        if (common::OB_ENTRY_NOT_EXIST == ret) {
          if (pos_ >= map_->bucket_num_) {
            ret = OB_ITER_END;
                         ...
                         map_->multi_get(cache_id_, pos_++, handle_list_))) {
