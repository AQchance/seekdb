    ...
    const ObITableReadInfo *rowkey_read_info)
{
  ...
  if(IS_INIT) {
    ...
    // Enable blockscan for all scans - the storage layer will determine if it's actually applicable
    // This allows batch mode when conditions are met (single SSTable, vectorized enabled, etc.)
